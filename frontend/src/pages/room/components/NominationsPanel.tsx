...
import { Search, Music2, Plus, Send, ListMusic, Vote, ChevronUp } from 'lucide-react';
...
export const NominationsPanel = ({ onNominate, onVote, onRequestSong }: Props) => {
    ...
    const [tab, setTab] = useState<'playlist' | 'votes'>('playlist');
    ...
    return (
        <div className="flex flex-col h-full p-4 gap-3 min-h-0">
            {/* Tab switcher: Playlist (creator queue) | Vote Queue (listener nominations) */}
            <div className="shrink-0 flex items-center gap-1 p-1 rounded-xl ring-1 ring-white/8" style={{ background: 'var(--ink-2)' }}>
                <button
                    onClick={() => setTab('playlist')}
                    className={cn(
                        'flex-1 flex items-center justify-center gap-1.5 h-9 rounded-lg text-[12px] press transition-all',
                        tab === 'playlist'
                            ? 'bg-white/10 text-white ring-1 ring-white/15'
                            : 'hover:bg-white/4 text-[var(--fg-2)]',
                    )}>
                    <ListMusic className="size-3.5" />
                    Playlist
                    <span className="mono text-[10px] tabular-nums px-1.5 py-0.5 rounded-full bg-white/8">
                        {upcoming.length}
                    </span>
                </button>
                <button
            ...
            {/* Tab content — flex-grows + scrolls internally */}
            <div className="flex-1 min-h-0 flex flex-col">
                {tab === 'playlist' ? (
                    <section className="flex-1 min-h-0 flex flex-col gap-1.5">
                        ...
                        {upcoming.length === 0 ? (
                            <div className="py-10 text-center rounded-xl ring-1 ring-dashed ring-white/8">
                                <Music2 className="size-6 mx-auto mb-2 opacity-20 text-white" />
                                <p className="text-[12px]" style={{ color: 'var(--fg-3)' }}>Playlist is empty</p>
                                <p className="mono text-[9px] mt-1" style={{ color: 'var(--fg-3)' }}>Switch to Vote Queue to nominate</p>
                            </div>
                        ) : (
                            <div className="flex-1 min-h-0 overflow-y-auto thin-scrollbar rounded-xl ring-1 ring-white/8 divide-y divide-white/5"
                    ...
                    <section className="flex-1 min-h-0 flex flex-col gap-1.5">
                        ...
                        {nominations.length === 0 ? (
                            <div className="py-10 text-center rounded-xl ring-1 ring-dashed ring-white/8">
                                <Vote className="size-6 mx-auto mb-2 opacity-20 text-white" />
                                <p className="text-[12px]" style={{ color: 'var(--fg-3)' }}>No nominations yet</p>
                                <p className="mono text-[9px] mt-1" style={{ color: 'var(--fg-3)' }}>Tap Nominate below to suggest a song</p>
                            </div>
                        ) : (
