...
import {
    ...
    ExternalLink, ShieldCheck, Link2, Download, Clock, TrendingUp, BrainCircuit,
    RefreshCw, WifiOff, PlayCircle, Database, Zap, BarChart2,
} from 'lucide-react';
import {
...
const SongsSection = () => {
    ...
    const handleUpload = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 1. Upload audio file to backend (which proxies to S3)
            const formData = new FormData();
            formData.append('audio', file);

            const { data: uploadData } = await axiosInstance.post('/admin/songs/upload', formData, {
            ...
            const { s3Key, uploadToken } = uploadData.data;
            setUploadProgress(100);

            // 2. Extract duration from uploaded file metadata
            const duration = await readAudioDuration(file);

            // 3. Finalize song creation in DB (server verifies upload token + S3 object)
...
const RecSysSection = () => {
    ...
    const fetchStatus = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(err) && err.response?.data?.offline) {
                setOffline(true);
            } else {
                toast.error('Failed to load RecSys status');
            }
        } finally {
            setLoading(false);
        }
    ...
    const triggerTrain = async (force = false) => {
        ...
        try {
            ...
            // Poll status after 3s to show is_training = true
    ...
    const lastTrained = status.model.trained_at
        ? new Date(status.model.trained_at).toLocaleString()
        : 'Never';
    ...
    const statCards = [
        {
            label: 'Model Version',
            value: neverTrained ? '—' : status.model.version.slice(0, 10) + '…',
            sub: `Trained: ${lastTrained}`,
            icon: BrainCircuit,
            color: 'text-violet-400',
        },
        {
            label: 'Cache Hit Rate',
            value: `${status.cache.hit_rate_pct}%`,
            sub: `${status.cache.users_cached.toLocaleString()} users cached`,
            icon: Zap,
            color: 'text-emerald-400',
        },
        {
            label: 'Total Requests',
            value: status.cache.total_requests.toLocaleString(),
            sub: `${status.cache.cache_hits.toLocaleString()} served from cache`,
            icon: BarChart2,
            color: 'text-sky-400',
        },
        {
            label: 'Training Scale',
            value: status.model.metrics.training_users.toLocaleString(),
            sub: `users · ${status.model.metrics.training_songs.toLocaleString()} songs`,
            icon: Database,
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-zinc-500 mt-1">
                        ALS collaborative filtering · nightly training at 02:00 UTC
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    <Button
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-zinc-400">
                    ...
                    {isTraining && (
                        <span className="ml-3 text-amber-400 text-xs font-medium">
                        ...
                        <div className="space-y-3 text-sm">
                            <div className="flex justify-between">
                                ...
                                <span className="text-white font-medium">
                                    {status.model.metrics.coverage != null
                                        ? `${(status.model.metrics.coverage * 100).toFixed(1)}%`
                                        : '—'}
                                </span>
                            </div>
                            <div className="flex justify-between">
                                ...
                                <span className="text-white font-medium">
                                    {status.model.metrics.precision_at_10 != null
                                        ? status.model.metrics.precision_at_10.toFixed(3)
                                        : '—'}
                                </span>
                            </div>
                            <div className="flex justify-between">
                                ...
                                <span className="text-white font-medium">
                                    {status.model.metrics.training_interactions.toLocaleString()}
                                </span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-zinc-500">Training time</span>
                                <span className="text-white font-medium">
                                    {status.model.duration_s != null ? `${status.model.duration_s.toFixed(1)}s` : '—'}
                                </span>
                            </div>
                        </div>
                    )}
                ...
                <div className="bg-zinc-900 border border-white/5 rounded-xl p-6 space-y-4">
                    ...
                    <div className="space-y-3 text-sm">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-white font-medium">
                                {status.scheduler.last_run_duration_s != null
                                    ? `${status.scheduler.last_run_duration_s.toFixed(1)}s`
                                    : '—'}
                            </span>
                        </div>
                        <div className="flex justify-between">
