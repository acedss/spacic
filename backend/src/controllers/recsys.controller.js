/**
 * RecSys proxy controller
 * ──────────────────────────────────────────────────────────────
 * Node.js sits in front of the Python RecSys service so the
 * admin panel never talks to Python directly. All auth (Clerk)
 * stays in Node; Python only accepts the internal API key.
 */
...
export const getRecSysStatus = async (req, res, next) => {
    try {
        ...
        if (!response.ok) {
            return res.status(502).json({ message: 'RecSys service returned an error' });
        }
        ...
        if (err.name === 'TimeoutError' || err.code === 'ECONNREFUSED') {
            return res.status(503).json({
                message: 'RecSys service is offline',
                offline: true,
            });
        }
        next(err);
...
// ── GET /api/recs/:userId ─────────────────────────────────────────────────────
// Used by the frontend (non-admin) to fetch personalized recommendations.
export const getUserRecs = async (req, res, next) => {
    try {
        ...
        if (!response.ok) {
            return res.status(502).json({ message: 'RecSys service error' });
        }
